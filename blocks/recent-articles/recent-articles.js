...
export default async function decorate(block) {
  ...
  for (let i = 0; i < amount; i++) {
    ...
    function renderImageHtml() {
      if (!magazine.image) return '';
      return createOptimizedPicture(magazine.image, magazine.title).innerHTML;
    }
    const innerh = html` <div class="article">
      <div class="image-wrapper">
        <a href="${magazine.path}"> ${renderImageHtml()} </a>
      </div>
    ...
    $item.insertAdjacentHTML('beforeend', innerh);
  }
